...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const NA = parseInt(String(req.query.accounts ?? "50"), 10);
  const NC = parseInt(String(req.query.contacts ?? "120"), 10);
  const ND = parseInt(String(req.query.deals ?? "160"), 10);
  const MA = parseInt(String(req.query.maxActs ?? "3"), 10);
  ...
  try {
    ...
    res.status(200).json({ reset, created:{ accounts:NA, contacts:NC, deals:ND, maxActivitiesPerDeal:MA }, summary:{ pipelines, stages:stagesCount, accounts:accountsCount, contacts:contactsCount, deals:dealsCount, activities:activitiesCount } });
  } catch (e:any) {
